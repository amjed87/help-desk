...
const HeaderLink: React.FC<{ item: HeaderItem }> = ({ item }) => {
  ...
  return (
    ...
    >
      <Link
        href={`/${item.href}`}
        className={`text-xl font-bold flex hover:text-primary transition-all duration-300 relative ${
          isActive
            ? "text-primary after:absolute after:w-8 after:h-1 after:bg-primary after:rounded-full after:-bottom-1"
            : "text-dark"
        }`}
      >
        {item.label}
